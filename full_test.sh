...
python -u main.py actnet RGB --arch resnet50 --num_segments 16 --npb --exp_header X --ada_reso_skip --policy_backbone resnet50 --offline_lstm_all --folder_suffix _self --frame_independent --batch-size 48 -j 16 --gpus 0 1 2 3 --test_from test_suite/exp2_res_uniform --uno_time | tee tmp_log.txt
OUTPUT2=`cat tmp_log.txt | tail -n 3`

#TODO 3. Test for Baseline LSTM (~71.2)
python -u main.py actnet RGB --arch resnet50 --num_segments 16 --npb --exp_header X --ada_reso_skip --policy_backbone resnet50 --backbone_list resnet50 --offline_lstm_all --folder_suffix _self --batch-size 48 -j 16  --gpus 0 1 2 3 --test_from test_suite/exp3_res_lstm --uno_time | tee tmp_log.txt
